        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.08fr_.92fr]">
          <section className="rounded-[28px] border border-[#d8d0c1] bg-[#fffdf8] p-6 shadow-[0_18px_55px_rgba(30,38,48,.08)] md:p-10">
            ...
            {step === 5 && <>
              ...
              <div className="mt-7 flex flex-wrap gap-3"><button onClick={() => setStep(4)} className="rounded-full border border-[#cfc5b4] px-5 py-3 text-sm font-bold">Back</button>{isPaid ? <button disabled={busy} onClick={async()=>{if(await save(5,"activate")) setNotice("Cerebro is active. Your guardrails are now the operating policy.");}} className="rounded-full bg-[#aa7a24] px-6 py-3 text-sm font-bold text-white">Activate Cerebro</button> : <Link href="/dashboard?upgrade=pro" className="rounded-full bg-[#aa7a24] px-6 py-3 text-sm font-bold text-white">See agent plans</Link>}<Link href="/dashboard" className="rounded-full px-5 py-3 text-sm font-bold text-[#536070]">Return to dashboard</Link></div>
            </>}
